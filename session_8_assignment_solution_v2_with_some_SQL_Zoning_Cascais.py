...
myselect='SELECT ST_AREA("suitableSinglepart"."geom") as area, "suitableSinglepart"."geom" as geometry ' 
myfrom=  'FROM "suitableSinglepart", "mainStreams" '
mywhere= 'WHERE ST_INTERSECTS(geometry, "mainStreams"."geom") and area >300000 '
myquery=myselect+myfrom+mywhere
myoutputdict={'area': QgsField('area',QVariant.Double),'geom':'POLYGON'}
...
############################################################# use SQL query to conclude 
# add layer to geopackage
add_layer_to_geopackage(fn,'mainStreams')
add_layer_to_geopackage(fn,'suitableSinglepart')
my_remove_layer('mainStreams')

# establish connection to geopackage
